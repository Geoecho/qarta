...
export const OrdersDashboard = () => {
    ...
    useEffect(() => {
        ...
        console.log("Admin Dashboard: Subscribing to Firestore...");
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            // Don't kill the UI, just maybe show error toast
        });

        return () => unsubscribe();
    }, []);

    // Backup Polling removed - Firestore socket is robust enough.

    const manualRefresh = () => {
        // No-op for Firestore usually, but we could re-mount if needed.
        // For visual feedback we can just show loading for a split second
        setLoading(true);
    ...
    const updateOrderStatus = async (orderId, status, minutes = null) => {
        try {
            ...
            // Optimistic update not strictly needed if onSnapshot is fast, but good for UX
    ...
    if (loading) return (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '400px', color: 'var(--text-muted)' }}>
            <div>Connecting to Live Orders...</div>
        </div>
    );
